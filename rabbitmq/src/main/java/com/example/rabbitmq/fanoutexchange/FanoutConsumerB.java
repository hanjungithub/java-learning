...
public class FanoutConsumerB {
    ...
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = RabbitmqConfig.QUEUE_B, durable = "true"),
            exchange = @Exchange(name = RabbitmqConfig.EXCHANGE_FANOUT, durable = "true", type = "fanout")))
    public void process(Message message, Channel channel) throws IOException {
        // 采用手动应答模式, 手动确认应答更为安全稳定
        channel.basicAck(message.getMessageProperties().getDeliveryTag(), true);
        log.info("fanout-exchange模式-receive-B: " + new String(message.getBody()));
    }
}
